import React, { useState, useEffect} from "react";
import { useNavigate } from "react-router-dom";

import TextFieldAuth from "../components/TextFieldAuth";
import Button from "../components/Button";
import TextField from "../components/TextField";
import Footer from "../components/Footer";
import Navbar from "../components/Navbar/Navbar";
import UserProfile from "../components/updateProfile";
import SelectLanguage from "./Modals/SelectLanguage";
import AccountDeletion_M from "./Modals/AccountDeletion";
import UpdateProfile from "./Modals/UpdateProfile";

import { IoShieldCheckmark } from "react-icons/io5";
import { IoPersonAdd } from "react-icons/io5";
import { IoOptions } from "react-icons/io5";
import { IoTrashBinSharp } from "react-icons/io5";
import FlagFr from "../assets/fr_flag.png";
import FlagEn from "../assets/en_flag.png";

import i18n from "../assets/Translation/i18n";
import {withTranslation} from 'react-i18next';

const Profile = () => {

    const profil_pic_path = "data:image/jpeg;base64,";
    const [selectedIdx, setSelectedIdx] = useState(0);

    return (
        <div>
            <Navbar />
            <div className="profile">
                <div className="profile-wrapper">
                    <NavigationCategories setSelectedIdx={setSelectedIdx}/>
                    <div className="selected-section-container">
                        {selectedIdx === 0 && <PersonalDetails />}
                        {selectedIdx === 1 && <Preferences />}
                        {selectedIdx === 2 && <AccountDeletion />}
                        {selectedIdx === 3 && <Security />}
                    </div>
                </div>
            </div>
            <Footer />
        </div>
    );
}

const NavigationCategories = (props) => {
    return (
        <div className="navigation-bar-container">
            <section className="navigation-bar__section" onClick={() => props.setSelectedIdx(0)}>
                <div className="section__icon-container">
                    <div className="section__icon">
                        <IoPersonAdd id="icon" />
                    </div>
                </div>
                <div className="section__description">
                    <h1>{i18n.t('profile_pd_title')}</h1>
                    <span id="description">{i18n.t('profile_pd_desc_1')}</span>
                    <span id="link">{i18n.t('profile_pd_link')}</span>
                </div>
            </section>
            <section className="navigation-bar__section" onClick={() => props.setSelectedIdx(3)}>
                <div className="section__icon-container">
                    <div className="section__icon">
                        <IoShieldCheckmark id="icon" />
                    </div>
                </div>
                <div className="section__description">
                    <h1>{i18n.t('profile_sec_title')}</h1>
                    <span id="description">{i18n.t('profile_sec_desc_1')}</span>
                    <span id="link">{i18n.t('profile_sec_link')}</span>
                </div>
            </section>
            <section className="navigation-bar__section" onClick={() => props.setSelectedIdx(1)}>
                <div className="section__icon-container">
                    <div className="section__icon">
                        <IoOptions id="icon" />
                    </div>
                </div>
                <div className="section__description">
                    <h1>{i18n.t('profile_pref_title')}</h1>
                    <span id="description">{i18n.t('profile_pref_desc_1')}</span>
                    <span id="link">{i18n.t('profile_pref_link')}</span>
                </div>
            </section>
            <section className="navigation-bar__section" onClick={() =>props.setSelectedIdx(2)}>
                <div className="section__icon-container">
                    <div className="section__icon">
                        <IoTrashBinSharp id="icon" />
                    </div>
                </div>
                <div className="section__description">
                    <h1>{i18n.t('profile_del_title')}</h1>
                    <span id="description">{i18n.t('profile_del_desc_1')}</span>
                    <span id="link">{i18n.t('profile_del_link')}</span>
                </div>
            </section>
        </div>
    );
}

const PersonalDetails = (props) => {
    const [displayModal, setDisplayModal] = useState(false);
    
    return (
        <div>
            <section className="selected-section__header">
                <>
                    <h1>{i18n.t('profile_pd_title')}</h1>
                    <span>{i18n.t('profile_pd_desc_2')}</span>
                </>
                <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/7/7e/Circle-icons-profile.svg/1200px-Circle-icons-profile.svg.png" alt="my profile picture"></img>
            </section>
            <div className="selected-section__content">
                <DataNode isLast={false} title={i18n.t('profile_pd_firstname')} content={localStorage.getItem("firstname")} onClick={() => setDisplayModal(true)} clicable={i18n.t('profile_edit')}/>
                <DataNode isLast={false} title={i18n.t('profile_pd_lastname')} content={localStorage.getItem("lastname")} onClick={() => setDisplayModal(true)} clicable={i18n.t('profile_edit')}/>
                <DataNode isLast={false} title={i18n.t('profile_pd_email')} content={localStorage.getItem("mail")} onClick={() => setDisplayModal(true)} clicable={i18n.t('profile_edit')}/>
                <DataNode isLast={true} title={i18n.t('profile_pd_raddr')} content={localStorage.getItem("address")} onClick={() => setDisplayModal(true)} clicable={i18n.t('profile_edit')}/>
            </div>
            {displayModal && <UpdateProfile CloseTab={() => setDisplayModal(false)} />}
        </div>
    );
}

const Preferences = (props) => {
    const [currentCountry, setCurrentCountry] = useState(i18n.language);
    const [displayLangSelection, setDisplayLangSelection] = useState(false);

    const languagesDatas = {
        'fr': {
            'country_name': 'Français',
            'flag': FlagFr,
        },
        'en-US': {
            'country_name': 'English',
            'flag': FlagEn,
        }
    }

    return (
        <div>
            <section className="selected-section__header">
                <h1>{i18n.t('profile_pref_title')}</h1>
                <span>{i18n.t('profile_pref_desc_2')}</span>
            </section>
            <div className="selected-section__content">
                <DataNode isLast={true} title={i18n.t('profile_pref_lg')} content={languagesDatas[currentCountry].country_name} clicable={i18n.t('profile_edit')} onClick={() => setDisplayLangSelection(true)}/>
            </div>
            {displayLangSelection && <SelectLanguage CloseTab={() => setDisplayLangSelection(false)}/>}
        </div>
    );
}

const Security = (props) => {

    return (
        <div>
            <section className="selected-section__header">
                <h1>{i18n.t('profile_sec_title')}</h1>
                <span>{i18n.t('profile_sec_desc_1')}</span>
            </section>
            <div className="selected-section__content">
                <DataNode isLast={true} title={i18n.t("profile_sec_cpwd")} content={i18n.t('profile_sec_cpwd_desc')} clicable={i18n.t('profile_edit')}/>
            </div>
        </div>
    );
}

const AccountDeletion = (props) => {
    const [displayDeleteModal, setDisplayDeleteModal] = useState(false);

    return (
        <div>
            <section className="selected-section__header">
                <h1>{i18n.t('profile_del_link')}</h1>
                <span>{i18n.t('profile_del_desc_2')}</span>
            </section>
            <div className="selected-section__content">
                <DataNode onClick={() => setDisplayDeleteModal(true)} isLast={true} id="account-deletion" title={i18n.t('profile_del_del')} content={i18n.t('profile_del_del_desc')} clicable={i18n.t('profile_del')}/>
            </div>
            {displayDeleteModal === true && <AccountDeletion_M CloseTab={() => setDisplayDeleteModal(false)} />}
        </div>
    );
}

const DataNode = (props) => {
    return(
        <div>
            <div className="datanode-container" id={props.id}>
                <span>{props.title}</span>
                <span id="content">{props.content}</span>
                <a href="#" onClick={props.onClick} name={props.name}>{props.clicable}</a>
            </div>
            {props.isLast === false && <hr />}
        </div>
    );
}

export default withTranslation('profile')(Profile);